...
export default function SeasoningCard({ id, name, description, imageUrl, recipeCount }: SeasoningCardProps) {
  return (
    <Link href={`/seasonings/${id}`}>
      <div className="cute-card p-5 h-full flex flex-col items-center text-center gap-3">
        ...
        {description && (
          <p className="text-sm line-clamp-2" style={{ color: "#8d6e63" }}>{description}</p>
        )}
